...
{
	glm::vec3 a = s.origin;
	glm::vec3 b = s.origin + (timestep * s.velocity);

	//Get the distance for bot ha and b from plane p
	float adist = glm::dot(a, p.n);
	float bdist = glm::dot(b, p.n);

	//intersects if on different sides of the plane
	if (adist * bdist < 0.0f) return true;

	//intersects if start or end position within radius from plane
	if (abs(adist) <= s.radius || abs(bdist) <= s.radius) return true;
...
void update()
{
	...
	if (isSpacePressed)
	{
		...
		else
		{
			...
			if (sphere.origin.x > 1)
				sphere.origin.x -= 2.0f;
		}
